...
class WorldDrawer 
{
	constructor(worldSize, terrainSharpness=8, terrainIrregularity=0.5)
	{
		...
		var geometry = new SphericalCubeGeometry(worldSize, terrainSharpness, terrainIrregularity);
...
var boxFS = `
	...
	void main()
	{
		...
		float moved_cos_theta = (max(0.0, cos_theta)*0.8+0.2); // movemos al intervalo (0.2, 0.8) para evitar oscuridad total
	
		gl_FragColor = I * moved_cos_theta * (Kd + refractionM * pow(max(0.0, cos_omega), alpha) / cos_theta);
